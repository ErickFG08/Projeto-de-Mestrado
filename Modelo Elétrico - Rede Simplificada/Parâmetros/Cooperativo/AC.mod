...
minimize fo_consumo_pfv: (sum {w in AC, t in Ot, f in Of} pot_pfv[w,t,f] * dT * tarifa_branca[t] * preco_energia);

# Consumo de energia das Baterias
minimize fo_consumo_bat: (sum {w in AC, t in Ot, f in Of} pot_bateria[w,t,f] * dT * tarifa_branca[t] * preco_energia);

# Consumo de energia daos Aparelhos
minimize fo_consumo_aparelhos:  (sum {w in AC, t in Ot, f in Of} (Pac[w,t,f] + pot_bateria[w,t,f] + pot_pfv[w,t,f]) * dT * tarifa_branca[t] * preco_energia);
...
## Qac ##
	...
	subject to Ire_AC_aprox_linear_a {w in AC, t in Ot : AC_Fase_a[w] == 1}:
	Pac[w,t,1] = Vrae[w] * Iac_re_a[w,t] + Viae[w] * Iac_im_a[w,t];
	
	subject to Ire_AC_aprox_linear_b {w in AC, t in Ot : AC_Fase_b[w] == 1}:
	Pac[w,t,2] = Vrbe[w] * Iac_re_b[w,t] + Vibe[w] * Iac_im_b[w,t];
	
	subject to Ire_AC_aprox_linear_c {w in AC, t in Ot : AC_Fase_c[w] == 1}:
	Pac[w,t,3] = Vrce[w] * Iac_re_c[w,t] + Vice[w] * Iac_im_c[w,t];
	
	subject to Ire_AC_aprox_linear_a0 {w in AC, t in Ot : AC_Fase_a[w] == 0}:
	0 = Vrae[w] * Iac_re_a[w,t] + Viae[w] * Iac_im_a[w,t];
	
	subject to Ire_AC_aprox_linear_b0 {w in AC, t in Ot : AC_Fase_b[w] == 0}:
	0 = Vrbe[w] * Iac_re_b[w,t] + Vibe[w] * Iac_im_b[w,t];
	
	subject to Ire_AC_aprox_linear_c0 {w in AC, t in Ot : AC_Fase_c[w] == 0}:
	0 = Vrce[w] * Iac_re_c[w,t] + Vice[w] * Iac_im_c[w,t];
	
	subject to Iim_AC_aprox_linear_a {w in AC, t in Ot : AC_Fase_a[w] == 1}:
	0 = -Vrae[w] * Iac_im_a[w,t] + Viae[w] * Iac_re_a[w,t];
	
	subject to Iim_AC_aprox_linear_b {w in AC, t in Ot : AC_Fase_b[w] == 1}:
	0 = -Vrbe[w] * Iac_im_b[w,t] + Vibe[w] * Iac_re_b[w,t];
	
	subject to Iim_AC_aprox_linear_c {w in AC, t in Ot : AC_Fase_c[w] == 1}:
...
# BEGIN BATTERY
	...
	subject to Iim_BAT_aprox_linear_a {b in BAT, t in Ot : BAT_Fase_a[b] == 1}:
	0 = -Vrae[b] * Ibat_im_a[b,t] + Viae[b] * Ibat_re_a[b,t];
	
	subject to Iim_BAT_aprox_linear_b {b in BAT, t in Ot : BAT_Fase_b[b] == 1}:
	0 = -Vrbe[b] * Ibat_im_b[b,t] + Vibe[b] * Ibat_re_b[b,t];
	
	subject to Iim_BAT_aprox_linear_c {b in BAT, t in Ot : BAT_Fase_c[b] == 1}:
...
# BEGIN PAINEL
	...
	subject to Ire_PFV_aprox_linear_a {p in PFV, t in Ot : PFV_Fase_a[p] == 1}:
	pot_pfv[p,t,1] = Vrae[p] * Ipfv_re_a[p,t] + Viae[p] * Ipfv_im_a[p,t];
	
	subject to Ire_PFV_aprox_linear_b {p in PFV, t in Ot : PFV_Fase_b[p] == 1}:
	pot_pfv[p,t,2] = Vrbe[p] * Ipfv_re_b[p,t] + Vibe[p] * Ipfv_im_b[p,t];
	
	subject to Ire_PFV_aprox_linear_c {p in PFV, t in Ot : PFV_Fase_c[p] == 1}:
	pot_pfv[p,t,3] = Vrce[p] * Ipfv_re_c[p,t] + Vice[p] * Ipfv_im_c[p,t];
	
	subject to Iim_PFV_aprox_linear_a {p in PFV, t in Ot : PFV_Fase_a[p] == 1}:
	0 = -Vrae[p] * Ipfv_im_a[p,t] + Viae[p] * Ipfv_re_a[p,t];
	
	subject to Iim_PFV_aprox_linear_b {p in PFV, t in Ot : PFV_Fase_b[p] == 1}:
	0 = -Vrbe[p] * Ipfv_im_b[p,t] + Vibe[p] * Ipfv_re_b[p,t];
	
	subject to Iim_PFV_aprox_linear_c {p in PFV, t in Ot : PFV_Fase_c[p] == 1}:
	0 = -Vrce[p] * Ipfv_im_c[p,t] + Vice[p] * Ipfv_re_c[p,t];

# END PAINEL
 
# -------- Limite de correntre pelos ramos (Linearização) ---------------------------------
# --------------- FASE A ------------------------------------------------------------------
