...
param dTemp = 2;
	...
	subject to Tin_2a {t in Ot, w in AC : t > 1 and AC_Fase_a[w] = 1}:
		Tin[w,t,1] <= (Tset_casa[w] + dTemp) + 999 * DR[w,t,1];
	
	subject to Tin_2b {t in Ot, w in AC : t > 1 and AC_Fase_b[w] = 1}:
		Tin[w,t,2] <= (Tset_casa[w] + dTemp) + 999 * DR[w,t,2];
	
	subject to Tin_2c {t in Ot, w in AC : t > 1 and AC_Fase_c[w] = 1}:
		Tin[w,t,3] <= (Tset_casa[w] + dTemp) + 999 * DR[w,t,3];
...
## Tin 3 ##
	...
	subject to restricao_DR_a {w in AC : AC_Fase_a[w] = 1}:
		sum{t in Ot} DR[w,t,1] <= 3;

	subject to restricao_DR_b {w in AC : AC_Fase_b[w] = 1}:
		sum{t in Ot} DR[w,t,2] <= 3;

	subject to restricao_DR_c {w in AC : AC_Fase_c[w] = 1}:
		sum{t in Ot} DR[w,t,3] <= 3;
...
## Demand Response ##
	...
	subject to restricao_DR_1{t in Ot, f in Of, w in AC}:
		0 <= DR[w,t,f] <= 2;
		
	subject to restricao_DR_3{t in Ot, f in Of, w in AC}:
		frequency_ac[w,t,f] <= 90 * (1 - 0.25 * DR[w,t,f]);
